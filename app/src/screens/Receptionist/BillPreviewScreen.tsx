...
export default function BillPreviewScreen({ navigation, route }: any) {
    const { patientName, patientId, appointmentId, services, total } = route.params;
    ...
    const handleSendBill = async () => {
        ...
        if (!parsedPatientId || Number.isNaN(parsedPatientId)) {
            Alert.alert('Invalid Patient', 'Please provide a valid numeric patient ID.');
            return;
        }

        const billItems = (services || []).map((service: any) => ({
            itemType: service.name.toLowerCase().includes('consult') ? 'CONSULTATION' :
                service.name.toLowerCase().includes('medicine') ? 'MEDICINE' :
                    service.name.toLowerCase().includes('test') ? 'LAB_TEST' : 'OTHER',
        ...
        const payload = {
            ...
            discountAmount: 0,
            taxAmount: Number((total * 0.1).toFixed(2)),
            paymentMethod: 'CASH',
            notes: 'Bill generated by receptionist',
            billItems,
        };
        ...
        try {
            ...
            navigation.navigate('PaymentAcceptance', {
                ...
                patientName: createdBill.patient?.fullName || patientName,
                patientId,
                total: Number(createdBill.totalAmount || total * 1.1),
                dueAmount: Number(createdBill.dueAmount || createdBill.totalAmount || total * 1.1),
            });
        } catch (error) {
            console.error('Failed to create bill:', error);
            Alert.alert('Error', 'Failed to save bill. Please check details and try again.');
        } finally {
